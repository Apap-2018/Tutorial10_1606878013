...
export class LabResult extends React.Component {
	...
	handleFormSubmit(e) {
		e.preventDefault()
		/** 
		 * TODO: Akses method labResult(requestBody) pada Appointment dan lakukan update state. 
		 */
		this.setState({
			loading: true
		})
		...
		Appointment.labResult(dataJson).then(response => {
			if(response.status === 200){
				this.setState({
					loading: false,
				})
				alert(`Sukses memasukkan hasil lab ${this.state.pasien.nama}`)
			}
			else{
				this.setState({
					loading: false
				})
				alert(`Gagal memasukkan hasil lab ${this.state.pasien.nama}`)
			}
		})
	}
